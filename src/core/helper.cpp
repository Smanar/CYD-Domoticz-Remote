...
void loadInfo(char* textChar, size_t textSize) {
    ...
    lv_snprintf(textChar + strlen(textChar), textSize - strlen(textChar),
        ...
        used_size / 1024, mon.used_pct, mon.frag_pct, mon.free_biggest_size / 1024,
        mon.max_used / 1024, mon.total_size / 1024);
    ...
    lv_snprintf(textChar + strlen(textChar), textSize - strlen(textChar),
        ...
        runningTime()/(3600*24), (runningTime()/3600)%24 ,
        (runningTime()/60)%60, runningTime()%60);
    ...
    if (WiFi.isConnected()) {
        lv_snprintf(textChar + strlen(textChar), textSize - strlen(textChar),
            "WiFi %s, IP %d.%d.%d.%d, websocket %s:%d %sconnected\n",
            global_config.wifiSSID, localIp[0], localIp[1], localIp[2], localIp[3],
            global_config.ServerHost, global_config.ServerPort, WS_Running()? "" : "NOT ");
    } else {
        lv_snprintf(textChar + strlen(textChar), textSize - strlen(textChar),
            "WiFi %s NOT connected\n",
            global_config.wifiSSID);
    }
