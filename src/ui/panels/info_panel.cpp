...
#ifdef OLD_DOMOTICZ
    snprintf(buff, 256, "/json.htm?type=devices&filter=%s&used=true&order=Name",text2);
#else
    //snprintf(buff, 256, "/json.htm?type=command&param=getdevices&used=true&displayhidden=1"); // TO TEST, need to be removed
    snprintf(buff, 256, "/json.htm?type=command&param=getdevices&filter=%s&used=true&order=Name",text2);
#endif
